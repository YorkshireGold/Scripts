/*
 This script is to send larger and larger data on params in responses are sending a malicious attack string in a parameter.
...
function scan(as, msg, param, value) {
	for (i = 0; i < attacks.length; i++) {
		...
		print(new_msg.getRequestHeader().getURI().toString())
		...
		if (StatusCode > 500 ) {	// Change to a test which detects the vulnerability
			NewFound = 'Code -> : ' + new_msg.getResponseHeader().getStatusCode() + ' |     -> ' + new_msg.getRequestHeader().getURI().toString() +' Time: ' + msg.getTimeSentMillis().toString()
			print("------------------------------------------")
			//print(NewFound)
